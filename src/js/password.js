document.addEventListener("DOMContentLoaded", () => {
    ...
    inputIcon.addEventListener("click", (e) => {
      ...
      inputIcon.setAttribute(
        'src',
        input.getAttribute('type') === 'password' ? '/build/img/eye-off.svg' : '/build/img/eye.svg'
      );
      input.setAttribute(
        'type',
    ...
    const passwordRule = (rule, ruleClass, strength) => {
      if (rule) {
        ...
        img.src = '../build/img/circle-check.svg';
      } else {
        const img = document.querySelector(`.${ruleClass} img`);
        img.src = '../build/img/circle-cross.svg';
      }
      return strength;
    };
    ...
    function checkStrength(password) {
      ...
      strength = passwordRule(
        password.length > 7, 'eight-character', strength
      );
